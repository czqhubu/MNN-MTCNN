...
static FaceDetect *m_face_detect_mnn;
...
std::string set_search_result (std::vector<FaceInfo> &res_obj)
{
...
Java_cn_sample_mnn_detect_Facetest_FaceDetectionModelInit(JNIEnv *env, jobject instance,
    ...
    LOGD("JNI model path = %s\n", faceDetectionModelPath);
    if (NULL == faceDetectionModelPath) {
        LOGD("faceDetectionModelPath null");
        return tRet;
    }
    ...
    // init face detect
    LOGD("========model_path %s\n", model_path.c_str());
    m_face_detect_mnn = new FaceDetect(model_path, 0.7f, 0.8f, 0.9f);
...
Java_cn_sample_mnn_detect_Facetest_FaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
    ...
    string result;
    std::vector<FaceInfo> face_info = m_face_detect_mnn->Detect(srcmatimg, minFaceSize, 3);
